...
use App\Models\Besoinpersonneladm;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\URL;
use Illuminate\Support\Facades\Auth;
use Encore\Admin\Controllers\AdminController;

class BesoinpersonneladmController extends AdminController
{
    /**
     ...
     * @var string
     */
    protected $title = 'Besoin en personnel administratif';
    ...
    {
        $grid = new Grid(new besoinpersonneladm());
        ...
        if (!(Auth::guard('admin')->user()->id == 0)) {
            $grid->model()->where('besoinpersonneladm.etablissementannees_id', Auth::guard('admin')->user()->idEtab);
        }
    ...
    {
        $show = new Show(besoinpersonneladm::findOrFail($id));
    ...
    {
        $form = new Form(new besoinpersonneladm());
...
.numberCircle:after {
  padding-right: 8px;
		...
		<div>
			...
			<span class="numberCircle" style="background-color: gray; font-weight: bold;">4</span>
...
');
        ...
        $form->select('fonctionpersonnels_id', __('Fonction du personnel'))->options($lesfonctions);

        $form->saved(function (Form $form) {
            // $idDonneeEtab=session('idDonneesEtab');
            // $idEtab=session('etablissementchoisi');
            //   return redirect('admin/personnels/create');
              return redirect('/admin/besoinpersonnelens/create');
