...
class Etablissement2emeSemestreController extends AdminController
{
    /**
    ...
    protected function grid()
    {
        ...
        $rapport3 = Carbon::parse($anneeScolaire->rapport3);
        $today = Carbon::now();
        ...
        switch ($role_id) {
            case 2:
                ...
                $grid->column('Document')->display(function () {
                    return '<a href="/rapportsemestre2/' . $this->id . '"  target="_blank" class="btn btn-primary">Télécharger </a>';
                });
				if ($daysDifference3 === 0) {
                    ...
                    $grid->column('Action')->display(function () {
                         session(['etablissementchoisi' => $this->id]);
                    return '<a href="/admin/etablissementdetails2eme/'.$this->id.'/edit"class="btn btn-success">Saisir les données</a>';
                    });
                }
                $grid->disableActions();
                ...
                break;
                case 5:
                    $grid->model()->where('directiondepartementales_id', Auth::guard('admin')->user()->idDr);
                    $grid->column('id', __('Id'));
