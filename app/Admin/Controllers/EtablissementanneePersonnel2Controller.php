...
class EtablissementanneePersonnel2Controller extends AdminController
{
    /**
...
.numberCircle:after {
  padding-right: 8px;
        ...
        <span >|</span>
        <span style="font-size: 14px";><a style="color:red !important;" href="/admin/etabanneepersonnel/' . $EtabAnnee->id . '/edit">Personnels</a></span>
        <span >|</span>
        ...
        $form->hasMany('personnelannees', __(''), function (Form\NestedForm $form) {
            ...
            $form->text('nbreheureeffectuee', __('Nombre d heure effectuee'));
            $form->text('nbreheureresponsabilite', __('Nombre heure de responsabilite'));
        });
		...
		// Callback après la soumission du formulaire
 /*   $form->submitted(function (Form $form) {
        // Récupération des données soumises
        $personnelannees = request()->get('personnelannees');

        if (!empty($personnelannees)) {
            // Fractionner les données en groupes de 10
            $chunks = array_chunk($personnelannees, 10);

            // Parcourir chaque groupe de 10
            foreach ($chunks as $chunk) {
                foreach ($chunk as $personnel) {
                    // Exemple de logique ou validation avant la sauvegarde
                    if (!isset($personnel['personnels_id'])) {
                        continue; // Ignorer les enregistrements invalides
                    }

                    // Assurez-vous de sauvegarder chaque élément dans la table appropriée
                    Personnelannee::create([
                        'etablissementannee_id' => $form->model()->id,
                        'personnels_id' => $personnel['personnels_id'] ?? null,
                        'quotahoraire' => $personnel['quotahoraire'] ?? null,
                        'nbreheureeffectuee' => $personnel['nbreheureeffectuee'] ?? null,
                    ]);
                }
            }
        }
