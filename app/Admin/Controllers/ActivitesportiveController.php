...
class ActivitesportiveController extends AdminController
{
    /**
     ...
     * @var string
     */
    protected $title = 'Activite sportive';
    ...
    {
        $grid = new Grid(new activitesportive());
        ...
        if (!(Auth::guard('admin')->user()->id == 0)) {
            $grid->model()->where('activitesportive.etablissementannees_id', Auth::guard('admin')->user()->idEtab);
        }
        ...
        // $grid->column('sport_id', __('Sport id'));
        $grid->sport_id()->display(function ($id) {
            $query = Sport::find($id);
            return $query->libellesport;
        });
    ...
    {
        $show = new Show(activitesportive::findOrFail($id));

        $show->field('id', __('Id'));
        $show->field('created_at', __('Created at'));
        $show->field('updated_at', __('Updated at'));
        $show->field('etablissementannees_id', __('Etablissement année'));
        $show->field('sport_id', __('Sport id'));
    ...
    {
        $form = new Form(new activitesportive());
...
.numberCircle:after {
  padding-right: 8px;
		...
		<div>
			...
			<span class="numberCircle" style="background-color: gray;font-weight: bold;">9</span>
...
');
        ...
        // $form->number('sport_id', __('Sport id'));
        $lessports = array();
        $sports = Sport::all();
        ...
        // $lessports = Sport::pluck('libellesport', 'id')->toArray();
        // $form->checkbox('sports','Sport')->options(Sport::all()->pluck('libellesport', 'id'));
        // $form->multipleSelect('sport_id', __('Sport'))->options($lessports);
        $form->multipleSelect('sport_id', __('Sport'))->options(Sport::all()->pluck('libellesport', 'id'));
