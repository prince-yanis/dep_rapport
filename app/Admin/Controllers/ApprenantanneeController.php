...
class ApprenantanneeController extends AdminController
{
    ...
    protected function grid()
    {
        ...
        if (in_array($current_role->role_id, [2])) {
            ...
            $grid->model()->where('apprenantannees.etablissementannees_id', '=', $EtabAnnee->id);
            ...
            $grid->apprenants_id("Nom & Prénoms")->display(function ($id) {
                ...
                if (!$query) {
                    return '<b>Non existant</b>'; // Or return an empty string
                }
    ...
    public function remplissagerentree(Request $request)
    {
        $EtabAnnee = DB::table('etablissementannees')
            ->where('anneescolaires_id', '=', Parametresglobaux::findOrFail(1)->anneescolaires_id)
            ->where('etablissements_id', '=', session('etablissementchoisi'))
        ...
        if (Classe::where('etablissementannees_id', $EtabAnnee->id)->count() < 1) {
            return back()->with('error', new MessageBag(['title' => 'Erreur', 'message' => "Vous n'avez pas renseigné de classes."]));
        }

		if (Apprenantannee::where('etablissementannees_id', $EtabAnnee->id)->where('classes_id', NULL)->count() > 0) {
            return back()->with('error', new MessageBag(['title' => 'Erreur', 'message' => "Veuillez affecter tous vos apprenants dans leurs classes respectives."]));
        }

        if (Personnelannee::where('etablissementannees_id', $EtabAnnee->id)->count() < 1) {
            return back()->with('error', new MessageBag(['title' => 'Erreur', 'message' => "Vous n'avez pas renseigné de personnel."]));
        }
		
        Etablissementannee::where('id', $EtabAnnee->id)->update([
            'niveaurentree' => 1,
        ]);
