...
class Plannings2Controller extends Controller
{
    ...
    // public function index(Request $request)
    {
        ...
        if (!($EtabAnnee)) {
            ...
            header("Location: https://enquete-deep.cpntic.com/admin/etablissements1er");
            exit;
        }
    ...
    public function getEvents(Request $request)
{
    ...
    public function getFilteredEvents(Request $request)
    {
        ...
        \Log::info('getFilteredEvents called with params:', $request->all());
        
        // Appliquer les filtres si ils sont présents et non vides
        if ($request->has('etablissementannees_id') && $request->etablissementannees_id !== '' && $request->etablissementannees_id !== null) {
        ...
        if ($request->has('personnels_id') && $request->personnels_id !== '' && $request->personnels_id !== null) {
            $query->where('personnels_id', $request->personnels_id);
            \Log::info('Filtering by personnels_id: ' . $request->personnels_id);
        }

        if ($request->has('classes_id') && $request->classes_id !== '' && $request->classes_id !== null) {
            $query->where('classes_id', $request->classes_id);
            \Log::info('Filtering by classes_id: ' . $request->classes_id);
        }

        if ($request->has('disciplines_id') && $request->disciplines_id !== '' && $request->disciplines_id !== null) {
        ...
        if ($plannings) {
            foreach ($plannings as $planning) {
                try {
                    $etablissement = $planning->etablissementannee && $planning->etablissementannee->etablissement ? $planning->etablissementannee->etablissement->denominationetab : '';
                    $discipline = $planning->discipline ? $planning->discipline->libellediscipline : '';
                    $professeur = $planning->personnel ? $planning->personnel->nom : '';
                    $classe = $planning->classe ? $planning->classe->denominationclasse : '';

                    // Formater pour FullCalendar v6
                    $events[] = [
                        'id' => $planning->id,
                    ...
                    \Log::info('Event added: ' . $planning->id . ' - ' . $discipline . ' - ' . $classe);
