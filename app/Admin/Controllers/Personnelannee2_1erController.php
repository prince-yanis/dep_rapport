...
class Personnelannee2_1erController extends AdminController
{
    /**
    ...
    protected function grid()
    {
        ...
        if (in_array($current_role->role_id, array(2))) {
            if (!$etablissement) {
                throw new \Exception('Établissement non trouvé pour l\'utilisateur actuel');
            }
            ...
            $grid->tools(function ($tools) {
                ...
                $tools->append('<a href="/admin/etabanneepoint/' . session('etablissementannees_id') . '/edit" class="btn btn-sm btn-success" >Suivant</a>');
            });
            ...
            $grid->column('Action')->display(function () {
                return '<a href="/admin/personnelannees2_1er/' . $this->id . '/edit" class= "btn btn-sm btn-primary" >Modifier</a>';
            });
        } else {
            ...
            $grid->tools(function ($tools) {
                ...
                $tools->append('<a href="/admin/etabanneeclasse1er/' . session('etablissementannees_id') . '/edit" class="btn btn-sm btn-success" >Suivant</a>');
            });
            $grid->column('id', __('Id'));
            // $grid->column('personnels_id', __('Personnels id'));
            $grid->etablissementannees_id("Année scolaire - Etablissement")->display(function ($etablissementannees_id) {
    ...
    protected function detail($id)
        ...
        protected function form()
        {
            ...
            $form->text('personnel.numeroautorisation', __('Numeroautorisation'));
            $form->date('personnel.dateautorisation', __('Dateautorisation'));
            $form->file('personnel.documentautorisation', "Uploader le document d'autorisation");
            $form->file('personnel.cv', "Uploader votre Curriculum Vitae");
            //////////////////////////////////////////////////////////////////////////////////
            ...
            $form->switch('cons_ens', __('Membre du conseil d\'enseignement'));
            ...
            $form->saving(function (Form $form) {
                if ($form->personnel) {
                    ...
                    $personnel = Personnel::where('telephone', $form->personnel['telephone'])->first();
                    ...
                    if (!$personnel) {
                        ...
                        $personnel->numeroautorisation = !empty($form->personnel['numeroautorisation']) ? $form->personnel['numeroautorisation'] : null;
                        $personnel->dateautorisation = !empty($form->personnel['dateautorisation']) ? $form->personnel['dateautorisation'] : null;
                        $personnel->documentautorisation = !empty($form->personnel['documentautorisation']) ? $form->personnel['documentautorisation'] : null;
                        $personnel->cv = !empty($form->personnel['cv']) ? $form->personnel['cv'] : null;
