...
use App\Models\Besoininfrastructure;
use Illuminate\Support\Facades\Auth;
use App\Models\Designationinfrastructure;
use App\Models\Infrastructure;
use Encore\Admin\Controllers\AdminController;

class BesoininfrastructureController extends AdminController
{
    /**
     ...
     * @var string
     */
    protected $title = 'Besoin en infrastructure';
    ...
    {
        $grid = new Grid(new besoininfrastructure());

        if (!(Auth::guard('admin')->user()->id == 0)) {
            $grid->model()->where('besoininfrastructures.etablissementannees_id', Auth::guard('admin')->user()->idEtab);
        }
        ...
        // $grid->column('designationinfrastructures_id', __('Designationinfrastructures id'));
        $grid->designationinfrastructures_id()->display(function ($id) {
            $query = Designationinfrastructure::find($id);
            return $query->libelleinfrastructure;
        });
    ...
    {
        $show = new Show(besoininfrastructure::findOrFail($id));

        $show->field('id', __('Id'));
        $show->field('quantite', __('Quantité'));
        $show->field('etablissementannees_id', __('Etablissementannees id'));
        $show->field('designationinfrastructures_id', __('Designationinfrastructures id'));
    ...
    {
        $form = new Form(new besoininfrastructure());
...
.numberCircle:after {
  padding-right: 8px;
		...
		<div>
			...
			<span style=" display: inline-block; width: 15px; border: 5px solid gray;border-radius: 0px; margin: 0px 0px 0px 0px;" ></span>
			<span class="numberCircle" style="background-color: gray;font-weight: bold;">7</span>
...
');
        ...
        $form->select('designationinfrastructures_id', __("Designation de l'infrastructure"))->options($lesdesignations);
