...
class EtablissementanneeBesoinformation1erController extends AdminController
{
    /**
...
.numberCircle:after {
  padding-right: 8px;
		...
		<div>
        <h1 style="text-align:center; text-transform:uppercase;">Besoin en formation</h1>
        ...
        <span >|</span>
        <span style="font-size: 14px";><a href="/admin/etabanneeapprenant1er/' . $EtabAnnee->id . '/edit">Apprenants</a></span>
        <span >|</span>
        ...
        <span >|</span>
        <span style="font-size: 14px";><a style="color:red !important;" href="/admin/etabanneebesoinformation1er/' . $EtabAnnee->id . '/edit">Besoin en Formation</a></span>
        <span >|</span>
        ...
        $form->hasMany('besoinformation', __(''), function (Form\NestedForm $form) {
            $EtabAnnee = DB::table('etablissementannees')
                ->where('anneescolaires_id', '=', Parametresglobaux::findOrFail(1)->anneescolaires_id)
                ->where('etablissements_id', '=', session('etablissementchoisi'))
                ->first();
            ...
            $personnels = Personnelannee::where('etablissementannees_id', '=', $EtabAnnee->id)
                ...
                ->join('disciplines', 'personnelannees.disciplines_id', '=', 'disciplines.id')
                ->join('statutpersonnel', 'personnelannees.statutpersonnel_id', '=', 'statutpersonnel.id')
                ->select('personnelannees.*',
                'personnels.matricule',
                'personnels.nom',
                'personnels.prenoms',
                'disciplines.libellediscipline',
                'statutpersonnel.libellestatutpers')
                ->get();
            foreach ($personnels as $personnel) {
                $lespersonnels[$personnel->id] = $personnel->matricule . " | " . $personnel->nom . " " . $personnel->prenoms . " | ". $personnel->libellediscipline ." | ". $personnel->libellestatutpers ;
            }
            $form->select('personnelannees_id', __('Selectionner le personnel concerné'))->options($lespersonnels);
        })->useTable();
        ...
        $form->saved(function (Form $form) {
            ...
            return redirect('admin/etabanneeprobleme1er/' . $EtabAnnee->id . '/edit');
        });
        return $form;
    }
}
