...
class Etablissement1erSemestreController extends AdminController
{
    /**
    ...
    protected function grid()
    {
        ...
        $rapport2 = Carbon::parse($anneeScolaire->rapport2);
        $today = Carbon::now();
        ...
        switch ($role_id) {
            case 2:
                ...
                $grid->column('Document')->display(function () {
                    return '<a href="/rapportsemestre1/' . $this->id . '"  target="_blank" class="btn btn-primary">Télécharger </a>';
                });
				...
				if ($daysDifference2 === 0) {
                    ...
                    $grid->column('Action')->display(function () {
                         session(['etablissementchoisi' => $this->id]);
                    return '<a href="/admin/etablissementdetails1er/'.$this->id.'/edit"class="btn btn-success">Saisir les données</a>';
                    });
                }
                $grid->disableActions();
